...
these commands
gcc A03_Keshav_2020EEB1177_22MCS506.c
cmd /c 'A03_Keshav_2020EEB1177_22MCS506 < input.txt > output.txt'
where A03_Keshav_2020EEB1177_22MCS506 is the exe file input.txt is the file containing input and output.txt where we want output
*/
...
// Function to add node into a list
struct List *newlistnode(int value, struct List *nex)
{
...
// Function to combine two normal lists
struct List *Combine(struct List *h1, struct List *h2)
{
    struct List *ans = NULL;
    for (struct List *i = h1; i != NULL; i = i->next)
    {
        ans = newlistnode(i->val, ans);
    }
    for (struct List *i = h2; i != NULL; i = i->next)
    {
        ans = newlistnode(i->val, ans);
    }
    ans = ReverseList(ans);
...
// Function to get a height of a Tree Node
...
// Function to get a Difference between height of left child and height of right child of a node
...
// Function to get a Predecessor for a node to delete
...
// Counting number of nodes and leaves in a tree
...
// Returns if a certain value node exists in a tree or not
...
// Finding Least Common Ancestor of a BST
...
// Printing path from ancestor to descendant
void PrintPath(struct Node *root, int j)
{
    int c = root->data;
    if(c == j)
        printf("%d\n", j);
        ...
        printf("%d R ", c);
        PrintPath(root->right, j);
    }
    else{
        printf("%d L ", c);
        PrintPath(root->left, j);
    }
}

// Checking possibilties for path from i to j
void PrintRoute(struct Node *root, int i, int j)
{
...
// List to store nodes of a Tree --> for implementation of Stack and Queue of TreeNodes
struct NodeList
{
    struct Node *node;
...
// Function to create new nodes for List storing TreeNodes
...
// Function to find the width of a tree
void WidthTrav(struct Node *root, int i, int *arr)
{
    if (root == NULL)
        return;
    arr[i]++;
    WidthTrav(root->left, i + 1, arr);
    WidthTrav(root->right, i + 1, arr);
}
...
// Traversal for computing diameter
...
// Returns sum of left most nodes excluding a leftmost leaf node
...
// Returns sum of right most nodes excluding a rightmost leaf node
...
// Finding Leaf Nodes for Boundary Traveral
struct List* FindLeaves(struct Node *root)
{
    if (root == NULL)
        return NULL;
    if (root->left == NULL && root->right == NULL)
        return newlistnode(root->data, NULL);
    struct List* l1 = FindLeaves(root->right);
    struct List* l2 = FindLeaves(root->left);
    struct List *ans = Combine(l1, l2);
    return ans;
}

// Boundary Traversal for Perimeter Function
void BoundaryTrav(struct Node *root)
{
    ...
    ans = ReverseList(ans);
    // Print(ans);
    // Print(FindLeaves(temp));
    ans = Combine(ans, FindLeaves(temp));
    temp = root;
...
// Function to destroy the current Tree and Free its space

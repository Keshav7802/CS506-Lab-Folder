...
class Binomial_Heap
{
    ...
    // heap is in increasing order of deg and
    // no two binomial trees have same deg in this heap
    list<Node *> adjust(list<Node *> _heap)
    {
        ...
        while (it1 != _heap.end())
        {
            ...
            // deg of three consecutive Binomial Tree are same
            // in heap
            ...
            // if deg of two Binomial Tree are same in heap
            else if ((*it1)->deg == (*it2)->deg)
            {
                Node *temp;
                *it1 = Merge(*it1, *it2);
...
int main(){
    int ch, key;
    Binomial_Heap obj = Binomial_Heap();

    list<Node*> _heap;

    // Insert data in the heap
    _heap = obj.insert(_heap, 10);
    _heap = obj.insert(_heap, 20);
    _heap = obj.insert(_heap, 30);

    cout << "Heap elements after insertion:\n";
    obj.printHeap(_heap);

    Node *temp = obj.getMin(_heap);
    cout << "\nMinimum element of heap "
         << temp->val << "\n";

    // Delete minimum element of heap
    _heap = obj.extractMin(_heap);
